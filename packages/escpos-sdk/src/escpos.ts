import { CartCalculationResult } from './cart-calculator.js';
...
export interface ReceiptMetadata {
  ...
  cashierName: string;
  customerName?: string;
  paymentTenders: Array<{ method: string; amount: number }>;
  isDuplicate?: boolean;
}
...
export class EscPosBuilder {
  ...
  textLine(str: string): EscPosBuilder {
    this.text(str);
    this.buffer.push(0x0a); // LF
    return this;
  }

  feed(lines = 1): EscPosBuilder {
  ...
  divider(char = '-'): EscPosBuilder {
    this.textLine(char.repeat(48));
    return this;
  }

  pulseCashDrawer(): EscPosBuilder {
...
export function generateEscPosReceiptBytecode(
  meta: ReceiptMetadata,
  cart: CartCalculationResult
): Uint8Array {
  ...
  builder.bold(false);
  if (meta.storeAddress) builder.textLine(meta.storeAddress);
  if (meta.storePhone) builder.textLine(`Tel: ${meta.storePhone}`);
  if (meta.taxNumber) builder.textLine(`Tax ID: ${meta.taxNumber}`);
  ...
  if (meta.customerName) builder.textLine(`Customer:  ${meta.customerName}`);
